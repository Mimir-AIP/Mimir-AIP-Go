...
func (cb *ChromemBackend) CreateCollection(ctx context.Context, name string, config CollectionConfig) error {
	fmt.Printf("[DEBUG] Before creation, collections: %v\n", cb.db.ListCollections())
	_, err := cb.db.CreateCollection(name, nil, cb.embedder)
	if err != nil {
		return fmt.Errorf("failed to create collection %s: %w", name, err)
	}
	// Ensure collection is registered
	_, err = cb.db.GetOrCreateCollection(name, nil, cb.embedder)
	if err != nil {
		return fmt.Errorf("failed to register collection %s: %w", name, err)
	}
	return nil
}
...
func (cb *ChromemBackend) ListCollections(ctx context.Context) ([]string, error) {
	fmt.Printf("[DEBUG] Listing collections: %v\n", cb.db.ListCollections())
	collections := cb.db.ListCollections()
	names := make([]string, 0, len(collections))
	for _, col := range collections {
		names = append(names, col.Name)
	}
	fmt.Printf("[DEBUG] Names slice to return: %#v\n", names)
	return names, nil
}
