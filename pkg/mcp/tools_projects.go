...
func registerProjectTools(s *server.MCPServer, m *MimirMCPServer) {
	...
	s.AddTool(
		mcp.NewTool("add_project_component",
			mcp.WithDescription("Associate a component (pipeline, ontology, ml_model, digital_twin, or storage) with a project"),
			...
			mcp.WithString("component_type",
				mcp.Required(),
				mcp.Description("Component type: pipeline, ontology, ml_model, digital_twin, or storage"),
			),
			mcp.WithString("component_id",
				mcp.Required(),
				mcp.Description("ID of the component to associate"),
			),
		),
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			projectID := req.GetString("project_id", "")
			componentType := req.GetString("component_type", "")
			componentID := req.GetString("component_id", "")
			if projectID == "" || componentType == "" || componentID == "" {
				return mcp.NewToolResultError("project_id, component_type, and component_id are required"), nil
			}
			var err error
			switch componentType {
			case "pipeline":
				err = m.projectSvc.AddPipeline(projectID, componentID)
			case "ontology":
				err = m.projectSvc.AddOntology(projectID, componentID)
			case "ml_model":
				err = m.projectSvc.AddMLModel(projectID, componentID)
			case "digital_twin":
				err = m.projectSvc.AddDigitalTwin(projectID, componentID)
			case "storage":
				err = m.projectSvc.AddStorage(projectID, componentID)
			default:
				return mcp.NewToolResultError("component_type must be one of: pipeline, ontology, ml_model, digital_twin, storage"), nil
			}
			if err != nil {
				return mcp.NewToolResultError(err.Error()), nil
			}
	...
	s.AddTool(
		mcp.NewTool("remove_project_component",
			mcp.WithDescription("Remove a component association from a project"),
			mcp.WithString("project_id",
				mcp.Required(),
				mcp.Description("Project ID"),
			),
			...
			mcp.WithString("component_id",
				mcp.Required(),
				mcp.Description("ID of the component to disassociate"),
			),
		),
		func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			projectID := req.GetString("project_id", "")
			...
			switch componentType {
			case "pipeline":
				err = m.projectSvc.RemovePipeline(projectID, componentID)
			case "ontology":
				err = m.projectSvc.RemoveOntology(projectID, componentID)
			case "ml_model":
				err = m.projectSvc.RemoveMLModel(projectID, componentID)
			case "digital_twin":
				err = m.projectSvc.RemoveDigitalTwin(projectID, componentID)
			case "storage":
				err = m.projectSvc.RemoveStorage(projectID, componentID)
			default:
				return mcp.NewToolResultError("component_type must be one of: pipeline, ontology, ml_model, digital_twin, storage"), nil
			}
			if err != nil {
